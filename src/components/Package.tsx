...
export default function Package({ pkg, showDownloads = false, animateDownloads = false, className = "" }: { pkg: PackageType, showDownloads?: boolean, animateDownloads?: boolean, className?: string }) {
  ...
  console.log(pkg);
  const bgText = pkg.dependencies ? Object.keys(pkg.dependencies).join(" ") : "";

  return (
    <div className={"relative overflow-hidden flex flex-col gap-4 items-center justify-center w-full h-full p-4 md:p-8 lg:p-12 bg-dark-blue " + className}>
      ...
      <div className="bg-decor">{bgText}</div>
    </div>
  )
}
