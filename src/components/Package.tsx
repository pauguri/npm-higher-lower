...
export default function Package({ pkg, guessable = false, className = "" }: { pkg: PackageType, guessable?: boolean, className?: string }) {
  return (
    <div className={"flex flex-col gap-4 items-center justify-center w-full h-full p-4 md:p-8 lg:p-12 bg-dark-blue " + className}>
      <div className="flex flex-col items-center gap-2 text-center">
        <h2 className="text-4xl">{pkg.package}</h2>
        <p className="text-xl font-thin">{pkg.description}</p>
      </div>
      {
        guessable ?
          (
          ...
          (
            <div className="flex flex-col items-center w-full text-yellow">
              <CountUp end={pkg.downloads} duration={2} separator="." className="text-5xl font-bold leading-none" />
